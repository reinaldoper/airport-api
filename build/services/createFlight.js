...
exports.getFlightByAirportId = exports.getFlightByPlaneId = exports.deleteFlight = exports.updateFlight = exports.getFlightById = exports.getAllFlights = void 0;
...
const planeRepo = data_source_1.AppDataSource.getRepository(Plane_1.Plane);
/**
 ...
 */
async function createFlight({ origem, destino, dataHoraPartida, dataHoraChegada, status, plane }) {
 ...
 */
const updateFlight = async (id, { origem, destino, dataHoraPartida, dataHoraChegada, status, plane }) => {
    const flight = await (0, exports.getFlightById)(id);
    if (!flight)
        throw new Error('Voo não encontrado');
    const origemEntity = await airportRepo.findOneBy({ id: origem });
    const destinoEntity = await airportRepo.findOneBy({ id: destino });
...
const getFlightByPlaneId = async (planeId) => {
    ...
    if (!flights || flights.length === 0)
        throw new Error('Nenhum voo encontrado para este avião');
    return flights;
};
exports.getFlightByPlaneId = getFlightByPlaneId;
/**
 ...
 * @returns {Promise<Flight[]>} A promise resolving to an array of Flight objects.
 */
const getFlightByAirportId = async (airportId) => {
    const flights = await flightRepo.find({
        where: { origem: { id: airportId } },
        relations: ['origem', 'destino', 'plane'],
    });
    if (!flights || flights.length === 0)
        throw new Error('Nenhum voo encontrado para este aeroporto');
    return flights;
};
exports.getFlightByAirportId = getFlightByAirportId;
